 ...
 */
exports.PATTERN_LINE_WITHOUT_CANDIDATES = /^([1-9]|[.\-*0]){81}$/;
/**
 ...
 */
exports.PATTERN_LINE_WITH_SPACES = /^([1-9]+[ ]){80}[1-9]+$/;
/**
 ...
 */
exports.PATTERN_LINE_FROM_HODOKU = /^:.*:.*:.*([\d.+]{81,}):.*:.*:.*$/;
/**
 ...
 */
exports.PATTERN_LINES_WITH_SPACES = /^(\s*[1-9]+){81}\s*$/;
/**
 ...
 * [3479][3679][8][37][47][5][2][1][349]`
 */
exports.PATTERN_LINES_WITH_BRACKETS = /^\s*((([{([])[1-9]+[}|)|\]]|[1-9]+){9}\s*){9}$/;
/**
 ...
 * ..8..521.`
 */
exports.PATTERN_NAKED_TABLE_WITHOUT_CANDIDATES = /^(\s*([1-9]|[.\-*0]){9}\s*){9}$/;
/**
 * Pattern for a table with no pencilmarks but empty cells instead (marked as
 ...
 * +---+---+---+`
 */
exports.PATTERN_TABLE_WITHOUT_CANDIDATES = /^\s*((\+-+){3}\+\s+(((\|([1-9]|[.\-*0]){3}){3}\|\s+){3})){3}(\+-+){3}\+\s*$/;
/**
 ...
 * +----------------------+----------------------+----------------------+`
 */
exports.PATTERN_TABLE = /^\s*((\+-+){3}\+\s+(((\|(\s+[1-9]+\s*){3}){3}\|\s+){3})){3}(\+-+){3}\+\s*$/;
/**
 ...
 * '----------------------'----------------------'----------------------'`
 */
exports.PATTERN_TABLE_SUDOPEDIA = /^\s*(\.-+){3}\.((\s+\|((\s\d+\s*){3}\|\s*){3}){3}\s+:(-+[+\s]){2}-+:\s*){2}(\s+\|((\s\d+\s*){3}\|\s*){3}){3}\s+('-+){3}'\s*$/;
function parseLine(line, delimiter, addCandidates, ignoredCellSymbols = []) {
    const cells = line.split(delimiter);
    const digits = new Map(remeda_1.pipe(cells, remeda_1.map.indexed((cell, index) => [index, cell]), remeda_1.filter((entry) => entry[1].length === 1 && !ramda_1.includes(entry[1])(ignoredCellSymbols)), remeda_1.map((entry) => [entry[0], parseInt(entry[1], 10)])));
    const candidates = new Map(remeda_1.pipe(cells, remeda_1.map.indexed((cell, index) => [index, cell]), remeda_1.filter((entry) => entry[1].length > 1), remeda_1.map((entry) => [entry[0], entry[1].split("").map((d) => parseInt(d, 10))])));
    return {
        digits,
        candidates: candidates.size === 0 && addCandidates ? candidate_1.getCandidates(digits) : candidates,
    };
}
/**
 ...
 * `);
 */
function parseGrid(stringGrid, addCandidates = false, onlyCleanString = false) {
    if (stringGrid.match(exports.PATTERN_LINE_WITHOUT_CANDIDATES)) {
        return parseLine(stringGrid, "", addCandidates, types_1.EMPTY_CELL_SYMBOLS);
    }
    else if (stringGrid.match(exports.PATTERN_LINE_WITH_SPACES)) {
        return parseLine(stringGrid, " ", addCandidates);
    }
    else if (!onlyCleanString) {
        // We may need to clean the string and parse it again
        if (stringGrid.match(exports.PATTERN_LINE_FROM_HODOKU)) {
            return parseGrid(stringGrid
                .replace(/^(:[^:]*:[^:]*:)/, "")
                .replace(/(:[^:]*:[^:]*:.*)$/, "")
                .replace(/(\+)/g, "")
                .replace(/(\s)/g, ""), addCandidates, 
            /* onlyCleanString: */ true);
        }
        else if (stringGrid.match(exports.PATTERN_LINES_WITH_SPACES)) {
            return parseGrid(stringGrid
                .replace(/[^1-9]/g, " ")
                .replace(/(^\s+)|(\s+$)/g, "")
                .replace(/\s{2,}/g, " "), addCandidates, 
            /* onlyCleanString: */ true);
        }
        else if (stringGrid.match(exports.PATTERN_LINES_WITH_BRACKETS)) {
            return parseGrid(stringGrid
        ...
        else if (stringGrid.match(exports.PATTERN_NAKED_TABLE_WITHOUT_CANDIDATES)) {
            return parseGrid(stringGrid.replace(/\s/g, ""), addCandidates);
        }
        else if (stringGrid.match(exports.PATTERN_TABLE_WITHOUT_CANDIDATES)) {
            return parseGrid(stringGrid.replace(/((\+-+){3}\+|\||\s)/g, ""), addCandidates);
        }
        else if (stringGrid.match(exports.PATTERN_TABLE)) {
            return parseGrid(stringGrid.replace(/((\+-+){3}\+|\||\s)/g, " "), addCandidates);
        }
        else if (stringGrid.match(exports.PATTERN_TABLE_SUDOPEDIA)) {
